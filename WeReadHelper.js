    ...
    for (let name in menu_all) {
      ...
      switch (name) {
        ...
        case 'play_turning_sound':
          // 添加新的
          menu_id[name] = GM_registerMenuCommand(' 翻页声：' + (value ? '✅' : '❌'), () => {
            menu_all[name] = !menu_all[name];
            GM_setValue('menu_all', menu_all);
            // 调用时触发，刷新菜单
            update_menu();
